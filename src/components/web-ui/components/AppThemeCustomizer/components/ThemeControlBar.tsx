...
import { oklchToHex } from "../../../theme-engine";
import {
	CARD_NAMES,
	CARD_OPTIONS,
...
import { buildPalette, type PalettePreset, TINT_MAX } from "../../../theme-engine";
import { cn } from "../../../lib/cn";
import type { ApplyResult } from "../hooks/use-apply-theme";
import { useApplyTheme } from "../hooks/use-apply-theme";
import { useResolvedTheme } from "../hooks/use-resolved-theme";
import type { ThemeDraft, ThemeDraftKit } from "../theme-draft";

/** One resolved token in the generated record - the sRGB hex a reader sees. */
export interface GeneratedSwatchToken {
	hex: string;
	oklch?: string;
}

/** The generated-palette record a project passes in - `GENERATED_PALETTES` from its own theme dir. */
export type GeneratedPalettes = Record<
	string,
	{
		dark: { tokens: Record<string, GeneratedSwatchToken> };
		light: { tokens: Record<string, GeneratedSwatchToken> };
	}
>;
...
export function ThemeControlBar({
	...
	const offPreset = !kit.isOnPreset(draft);
	const { apply: applyTheme, state: applyState } = useApplyTheme(onApply);
	 ...
	 */
	const output = kit.configOutput(draft);
	const dirty = kit.isDirty(draft);
				...
				<Field label="Theme">
					<div className="flex items-center gap-2">
						...
						>
							...
							>
								{presets.map((preset) => {
									const tokens = generatedPalettes[preset.name]?.[theme]?.tokens;
