...
import { Kbd, Separator, Slider, Spinner } from "@heroui/react";
import {
	Apple,
	Chrome,
	FilePlus2,
		...
		>
			...
			<div className="space-y-4">
				<Panel>
					...
					{/* `orientation="vertical"` on purpose: this narrow column wants one
					    tile per row and a stable layout that does not reflow with the
					    viewport, so the preview reads the same at every width. The `auto`
					    default is right for a real form; a preview board is not one. */}
					<AppRadioGroup
						control={control}
						items={[
			...
			<div className="space-y-4">
				<Panel>
					...
					{/* `length={6}` gives the 3+3 split AppInputOTP already knows. The
					    child selector keeps the row centred: `.input-otp` is `flex
					    w-full`, so it fills the panel and its groups sit left unless the
					    justify is set on the OTP element itself rather than a wrapper. */}
					<AppInputOTP
						className="items-center [&_.input-otp]:justify-center"
